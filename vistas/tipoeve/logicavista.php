...
$diccionario = array('titulos'=>array('registrar'=>'Agregar Tipo de Evento',
									  'verdatos'=>'Tabla de Tipos de Eventos',
									  'modificar'=>'Modificar Tipo de Evento '));
	...
	for ($i=0; $i < $num_filas ; $i++) {
		$html2 .= "<tr>
					<td style='text-align: center;'>
					    <a href='/investigacion/tipoeve/buscar/?id=".$data[$i]['id_tipo']."' class='btn btn-info btn-xs'><i class='fa fa-pencil'></i> Editar</a>
					    <a href='#' id='".$data[$i]['id_tipo']."' class='btn btn-danger btn-xs'><i class='fa fa-eraser'></i> Eliminar</a>
		...
		$html2 .= "
						<script type='text/javascript'>
						        ...
						        $('".$data[$i]['id_tipo']."').onclick = function () {
						            reset();
						            alertify.confirm('¿Desea Eliminar Este Tipo?: <b>".$data[$i]['des_tipo']."</b>', function (e) {
						                if (e) {
						                	location = '/investigacion/tipoeve/eliminar/?id=".$data[$i]['id_tipo']."';
						                } else {
...
if ($vista=='registrar'){
	$html = str_replace('{contenido}',  buscar_plantilla('registrar','tipoeve'), $html);
	$html = str_replace('{subtitulo}', $diccionario['titulos'][$vista], $html);

}

if ($vista=='verdatos'){
	$html = str_replace('{contenido}',  buscar_plantilla('verdatos','tipoeve'), $html);
	$html = str_replace('{subtitulo}', $diccionario['titulos'][$vista], $html);
	$html = llenar_tabla($html, $data);
}

if ($vista=='modificar'){
	$html = str_replace('{contenido}',  buscar_plantilla('modificar','tipoeve'), $html);
	$html = str_replace('{subtitulo}', $diccionario['titulos'][$vista], $html);
