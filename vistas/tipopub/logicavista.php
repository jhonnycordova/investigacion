...
$diccionario = array('titulos'=>array('registrar'=>'Agregar Tipo de Público',
									  'verdatos'=>'Tabla de Tipos de Públicos',
									  'modificar'=>'Modificar Tipo de Público '));
	...
	for ($i=0; $i < $num_filas ; $i++) {
		$html2 .= "<tr>
					<td style='text-align: center;'>
					    <a href='/investigacion/tipopub/buscar/?id=".$data[$i]['id_publico']."' class='btn btn-info btn-xs'><i class='fa fa-pencil'></i> Editar</a>
					    <a href='#' id='".$data[$i]['id_publico']."' class='btn btn-danger btn-xs'><i class='fa fa-eraser'></i> Eliminar</a>
		...
		$html2 .= "
						<script type='text/javascript'>
						        ...
						        $('".$data[$i]['id_publico']."').onclick = function () {
						            reset();
						            alertify.confirm('¿Desea Eliminar Este Tipo?: <b>".$data[$i]['des_publico']."</b>', function (e) {
						                if (e) {
						                	location = '/investigacion/tipopub/eliminar/?id=".$data[$i]['id_publico']."';
						                } else {
...
if ($vista=='registrar'){
	$html = str_replace('{contenido}',  buscar_plantilla('registrar','tipopub'), $html);
	$html = str_replace('{subtitulo}', $diccionario['titulos'][$vista], $html);

}

if ($vista=='verdatos'){
	$html = str_replace('{contenido}',  buscar_plantilla('verdatos','tipopub'), $html);
	$html = str_replace('{subtitulo}', $diccionario['titulos'][$vista], $html);
	$html = llenar_tabla($html, $data);
}

if ($vista=='modificar'){
	$html = str_replace('{contenido}',  buscar_plantilla('modificar','tipopub'), $html);
	$html = str_replace('{subtitulo}', $diccionario['titulos'][$vista], $html);
