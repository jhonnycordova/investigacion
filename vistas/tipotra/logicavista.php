...
$diccionario = array('titulos'=>array('registrar'=>'Agregar Tipo de Trabajo',
									  'verdatos'=>'Tabla de Tipos de Trabajos',
									  'modificar'=>'Modificar Tipo de Trabajo '));
	...
	for ($i=0; $i < $num_filas ; $i++) {
		$html2 .= "<tr>
					<td style='text-align: center;'>
					    <a href='/investigacion/tipotra/buscar/?id=".$data[$i]['id_tipotra']."' class='btn btn-info btn-xs'><i class='fa fa-pencil'></i> Editar</a>
					    <a href='#' id='".$data[$i]['id_tipotra']."' class='btn btn-danger btn-xs'><i class='fa fa-eraser'></i> Eliminar</a>
		...
		$html2 .= "
						<script type='text/javascript'>
						        ...
						        $('".$data[$i]['id_tipotra']."').onclick = function () {
						            reset();
						            alertify.confirm('¿Desea Eliminar Este Tipo?: <b>".$data[$i]['des_trabajo']."</b>', function (e) {
						                if (e) {
						                	location = '/investigacion/tipotra/eliminar/?id=".$data[$i]['id_tipotra']."';
						                } else {
...
if ($vista=='registrar'){
	$html = str_replace('{contenido}',  buscar_plantilla('registrar','tipotra'), $html);
	$html = str_replace('{subtitulo}', $diccionario['titulos'][$vista], $html);

}

if ($vista=='verdatos'){
	$html = str_replace('{contenido}',  buscar_plantilla('verdatos','tipotra'), $html);
	$html = str_replace('{subtitulo}', $diccionario['titulos'][$vista], $html);
	$html = llenar_tabla($html, $data);
}

if ($vista=='modificar'){
	$html = str_replace('{contenido}',  buscar_plantilla('modificar','tipotra'), $html);
	$html = str_replace('{subtitulo}', $diccionario['titulos'][$vista], $html);
