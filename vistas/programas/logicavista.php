...
$diccionario = array('titulos'=>array('verdatos'=>'Tabla de Programas',
									  'modificar'=>'Modificar Programa '));
	...
	for ($i=0; $i < $num_filas ; $i++) {
		$html2 .= "<tr>
					<td style='text-align: center;'>
					    <a href='/investigacion/programas/buscar/?id=".$data[$i]['id_prog']."' class='btn btn-info btn-xs'><i class='fa fa-pencil'></i> Editar</a>
					    

					</td>
					<td >{des_prog}</td>
					<td ><textarea  class='form-control' rows='3' cols='30' readonly >{mision_prog}</textarea></td>
					<td ><textarea  class='form-control' rows='3' cols='30' readonly >{vision_prog}</textarea></td>";
		$html2 .= "</tr>";
...
if ($vista=='verdatos'){
	$html = str_replace('{contenido}',  buscar_plantilla('verdatos','programas'), $html);
	$html = str_replace('{subtitulo}', $diccionario['titulos'][$vista], $html);
	$html = llenar_tabla($html, $data);
}

if ($vista=='modificar'){
	$html = str_replace('{contenido}',  buscar_plantilla('modificar','programas'), $html);
	$html = str_replace('{subtitulo}', $diccionario['titulos'][$vista], $html);
