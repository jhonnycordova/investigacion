...
$diccionario = array('titulos'=>array('registrar'=>'Agregar Nuevo Tipo de Proyecto',
									  'verdatos'=>'Tabla de Tipos de Proyecto',
									  'modificar'=>'Modificar Tipo de Proyecto '));
	...
	for ($i=0; $i < $num_filas ; $i++) {
		$html2 .= "<tr>
					<td style='text-align: center;'>
					    <a href='/investigacion/tipoproy/buscar/?id=".$data[$i]['id_tipo_pro']."' class='btn btn-info btn-xs'><i class='fa fa-pencil'></i> Editar</a>
					    <a href='#' id='".$data[$i]['id_tipo_pro']."' class='btn btn-danger btn-xs'><i class='fa fa-eraser'></i> Eliminar</a>
		...
		$html2 .= "
						<script type='text/javascript'>
						        ...
						        $('".$data[$i]['id_tipo_pro']."').onclick = function () {
						            reset();
						            alertify.confirm('¿Desea Eliminar el Cargo?: <b>".$data[$i]['des_tipo_pro']."?</b>', function (e) {
						                if (e) {
						                	location = '/investigacion/tipoproy/eliminar/?id=".$data[$i]['id_tipo_pro']."';
						                } else {
...
if ($vista=='registrar'){
	$html = str_replace('{contenido}',  buscar_plantilla('registrar','tipoproy'), $html);
	$html = str_replace('{subtitulo}', $diccionario['titulos'][$vista], $html);

}

if ($vista=='verdatos'){
	$html = str_replace('{contenido}',  buscar_plantilla('verdatos','tipoproy'), $html);
	$html = str_replace('{subtitulo}', $diccionario['titulos'][$vista], $html);
	$html = llenar_tabla($html, $data);
}

if ($vista=='modificar'){
	$html = str_replace('{contenido}',  buscar_plantilla('modificar','tipoproy'), $html);
	$html = str_replace('{subtitulo}', $diccionario['titulos'][$vista], $html);
